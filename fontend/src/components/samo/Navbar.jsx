...
const Samo_Navbar = ({ onLogout, role }) => {
    ...
    <header className="bg-[#ffcd08] text-gray-900 shadow-md sticky top-0 z-50 font-sans">
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          {/* --- Desktop Menu (เมนูเฉพาะของ Samo) --- */}
          <div className="hidden lg:flex items-center gap-2 bg-black/5 p-1.5 rounded-full">
            ...
            <Link to="/navbar_request" className={`px-6 py-2 rounded-full transition-all duration-300 ${isActive('/navbar_request')}`}>
              คำขอ
            </Link>
            <Link to="/manage_supply" className={`px-6 py-2 rounded-full transition-all duration-300 ${isActive('/manage_supply')}`}>
              จัดการพัสดุ
            </Link>
            <Link to="/outstanding" className={`px-6 py-2 rounded-full transition-all duration-300 ${isActive('/outstanding')}`}>
              รายการค้าง
            </Link>
            <Link to="/history_all" className={`px-6 py-2 rounded-full transition-all duration-300 ${isActive('/history_all')}`}>
              ประวัติรายการ
            </Link>
          </div>
      ...
      <div className={`lg:hidden absolute w-full bg-[#ffcd08] shadow-xl border-t border-black/5 transition-all duration-300 ease-in-out overflow-hidden ${isOpen ? "max-h-screen opacity-100" : "max-h-0 opacity-0"}`}>
        <div className="px-4 pt-4 pb-6 space-y-3 flex flex-col items-center">
          ...
          <Link to="/navbar_request" onClick={() => setIsOpen(false)} className="w-full text-center py-3 font-bold hover:bg-white/20 rounded-lg transition-colors">คำขอ</Link>
          <Link to="/manage_supply" onClick={() => setIsOpen(false)} className="w-full text-center py-3 font-bold hover:bg-white/20 rounded-lg transition-colors">จัดการพัสดุ</Link>
          <Link to="/outstanding" onClick={() => setIsOpen(false)} className="w-full text-center py-3 font-bold hover:bg-white/20 rounded-lg transition-colors">รายการค้าง</Link>
          <Link to="/history_all" onClick={() => setIsOpen(false)} className="w-full text-center py-3 font-bold hover:bg-white/20 rounded-lg transition-colors">ประวัติรายการ</Link>
